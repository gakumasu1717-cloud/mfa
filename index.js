...
const DebugLog = {
    ...
    _buildEntry(e, idx) {
        ...
        if (e.msg.length > FOLD_THRESHOLD) {
            ...
            return `<div style="margin:1px 0;">${header}<span class="mfa-fold" data-idx="${idx}"><span class="mfa-fold-short" style="color:#ddd;">${preview}<span class="mfa-fold-btn" data-action="expand" style="color:#64b5f6;cursor:pointer;margin-left:4px;">▼ 펼치기</span></span><span class="mfa-fold-long" style="display:none;color:#ddd;">${escaped}<br><span class="mfa-fold-btn" data-action="collapse" style="color:#64b5f6;cursor:pointer;">▲ 접기</span></span></span></div>`;
        }
        return `<div style="margin:1px 0;">${header}<span style="color:#ddd;">${escaped}</span></div>`;
    },

    render() {
        const el = $("#mfa_log_content");
        if (!el.length) return;
    ...
    clear() { this.entries = []; this._lastRenderedCount = 0; $("#mfa_log_content").html(""); },
};
    ...
    async interceptAndSend(requestBody) {
        ...
        if (isAnthropic && response.ok) {
            try {
                ...
                return new Response(JSON.stringify({
                    choices: [{ message: { role: "assistant", content: `[MFA] 응답 변환 오류: ${e.message}` }, index: 0, finish_reason: "stop" }],
    ...
    async convertAnthropicResponse(response, isStream) {
        if (isStream) {
            ...
            try {
                ...
                return new Response(JSON.stringify({
                    choices: [{ message: { role: "assistant", content: "[MFA] 응답 파싱 실패" }, index: 0, finish_reason: "stop" }],
                }), { status: 200, headers: { "Content-Type": "application/json" } });
            }
...
jQuery(async () => {
    ...
    $("#mfa_enabled").on("change", function () {
        ...
        s.enabled ? toastr.success("[MFA] 활성화") : toastr.info("[MFA] 비활성화");
        updateStatus();
    });

    $("#mfa_endpoint").on("change", function () {
        const s = getSettings();
        s.endpoint = $(this).val();
        saveSettings();
        ...
        $(".mfa-passthrough-only").toggle(s.endpoint === "passthrough");
        updateStatus();
    });

    // adaptive thinking
    $("#mfa_adaptive_thinking").on("change", function () {
        const s = getSettings();
        s.adaptiveThinking = $(this).prop("checked");
        saveSettings();
        $(".mfa-budget-row").toggle(!s.adaptiveThinking);
        DebugLog.info("Adaptive Thinking:", s.adaptiveThinking ? "ON" : "OFF");
    });

    // thinking budget
    $("#mfa_thinking_budget").on("change", function () {
        const s = getSettings();
    ...
    $("#mfa_path_correction").on("change", function () {
    ...
    $("#mfa_trim_assistant").on("change", function () {
        const s = getSettings(); s.trimAssistant = $(this).prop("checked"); saveSettings();
        DebugLog.info("assistant trim:", s.trimAssistant ? "ON" : "OFF");
    });
    $("#mfa_force_last_user").on("change", function () {
        const s = getSettings(); s.forceLastUser = $(this).prop("checked"); saveSettings();
        DebugLog.info("마지막 user 강제:", s.forceLastUser ? "ON" : "OFF");
    });
    $("#mfa_basic_auth_compat").on("change", function () {
        const s = getSettings(); s.basicAuthCompat = $(this).prop("checked"); saveSettings();
        DebugLog.info("basicAuth:", s.basicAuthCompat ? "ON" : "OFF");
    });
    $("#mfa_debug_log").on("change", function () {
        const s = getSettings(); s.debugLog = $(this).prop("checked"); saveSettings();
        s.debugLog ? $("#mfa_log_panel").slideDown(150) && DebugLog.render() : $("#mfa_log_panel").slideUp(150);
    });
    $("#mfa_clear_log").on("click", () => { DebugLog.clear(); toastr.info("[MFA] 로그 초기화"); });

    // 접기/펼치기 이벤트 위임
    $("#mfa_log_content").on("click", ".mfa-fold-btn", function () {
        const fold = $(this).closest(".mfa-fold");
        const action = $(this).data("action");
        if (action === "expand") {
            fold.find(".mfa-fold-short").hide();
            fold.find(".mfa-fold-long").show();
        } else {
            fold.find(".mfa-fold-long").hide();
            fold.find(".mfa-fold-short").show();
        }
    });

    // 맨 아래로 스크롤
    $("#mfa_scroll_bottom").on("click", () => {
        const el = $("#mfa_log_content");
        el.scrollTop(el[0]?.scrollHeight || 0);
    });

    // 모두 접기
    $("#mfa_fold_all").on("click", function () {
        const el = $("#mfa_log_content");
        const isAllFolded = el.find(".mfa-fold-long:visible").length === 0;
        if (isAllFolded) {
            // 모두 펼치기
            el.find(".mfa-fold-short").hide();
            el.find(".mfa-fold-long").show();
            $(this).val("📁 모두 접기");
        } else {
            // 모두 접기
            el.find(".mfa-fold-long").hide();
            el.find(".mfa-fold-short").show();
            $(this).val("📂 모두 펼치기");
        }
    });
    ...
    $("#mfa_token_select").on("change", function () {
        const s = getSettings();
        const idx = $(this).val();
        if (idx === "" || idx === null) {
            s.token = "";
    ...
    $("#mfa_token_add").on("click", function () {
        const s = getSettings();
        const name = $("#mfa_token_name").val().trim();
        const value = $("#mfa_token_value").val().trim();
        if (!value) { toastr.warning("[MFA] 토큰을 입력하세요"); return; }
        ...
        $("#mfa_token_name").val("");
        $("#mfa_token_value").val("");
        toastr.success(`[MFA] 토큰 추가: ${name || "토큰"}`);
        DebugLog.info(`토큰 추가: ${name} (${value.substring(0, 10)}...)`);
    });

    $("#mfa_token_delete").on("click", function () {
        const s = getSettings();
        const idx = $("#mfa_token_select").val();
        if (idx === "" || idx === null) { toastr.warning("[MFA] 삭제할 토큰을 선택하세요"); return; }
        ...
        renderTokenSelect();
        updateStatus();
        toastr.info(`[MFA] 토큰 삭제: ${removed.name}`);
    ...
    $("#mfa_enabled").prop("checked", s.enabled);
    $("#mfa_endpoint").val(s.endpoint);
    renderTokenSelect();
    $("#mfa_thinking_budget").val(s.thinkingBudget || 10000);
    $("#mfa_adaptive_thinking").prop("checked", !!s.adaptiveThinking);
    $(".mfa-budget-row").toggle(!s.adaptiveThinking);

    $("#mfa_remove_prefill").prop("checked", s.removePrefill);
    $("#mfa_trim_assistant").prop("checked", s.trimAssistant);
    $("#mfa_force_last_user").prop("checked", s.forceLastUser);
    ...
    $(".mfa-thinking-only").toggle(s.endpoint === "anthropic-thinking");
    $(".mfa-passthrough-only").toggle(s.endpoint === "passthrough");
    if (!s.debugLog) $("#mfa_log_panel").hide();

    if (s.enabled) Interceptor.install();
    updateStatus();
    DebugLog.info("MFA 로드 완료");
});
